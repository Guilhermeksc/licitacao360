...
class PlanejamentoWidget(QMainWindow):
    ...
    def carregar_tabela_de_arquivo(self):
        ...
        # Iterar sobre as linhas do modelo carregado
        for row in range(self.model.rowCount()):
            # Criar item_data para a linha atual
            item_data = {
                'tipo': self.model.data(self.model.index(row, self.model.fieldIndex('tipo'))),
                'numero': self.model.data(self.model.index(row, self.model.fieldIndex('numero'))),
                'ano': self.model.data(self.model.index(row, self.model.fieldIndex('ano'))),
                'objeto': self.model.data(self.model.index(row, self.model.fieldIndex('objeto'))),
                'sigla_om': self.model.data(self.model.index(row, self.model.fieldIndex('sigla_om'))),
                'material_servico': self.model.data(self.model.index(row, self.model.fieldIndex('material_servico'))),
                'id_processo': self.model.data(self.model.index(row, self.model.fieldIndex('id_processo'))),
                'nup': self.model.data(self.model.index(row, self.model.fieldIndex('nup'))),
                'orgao_responsavel': self.model.data(self.model.index(row, self.model.fieldIndex('orgao_responsavel'))),
                'uasg': self.model.data(self.model.index(row, self.model.fieldIndex('uasg')))
            }
            print(f"Processando id_processo: {item_data['id_processo']}")

            # Salvar o item no banco de dados e no controle de prazos
            self.save_to_database(item_data)
            self.save_to_control_prazos(item_data['id_processo'])
