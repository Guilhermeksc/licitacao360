...
class GerarDFD(QDialog):
    def __init__(self, main_app, config_manager, df_registro, parent=None):
        ...
        self.setWindowTitle("Documento de Formalização de Demanda (DFD)")
    ...
    def setupGrupoSIGDEM(self):
        ...
        sinopse_text = (f"Documento de Formalização de Demanda (DFD) referente ao {self.tipo} nº {self.numero}/{self.ano}, para {descricao_servico} {self.objeto}\n"
    ...
    def editarTemplate(self):
        template_path = TEMPLATE_PLANEJAMENTO_DIR / "template_dfd.docx"
        try:
    ...
    def gerarDocumento(self, tipo="docx"):
        ...
        template_path = TEMPLATE_PLANEJAMENTO_DIR / f"template_dfd.{tipo}"
        ...
        subpasta_autorizacao = f"{id_processo_novo} - DFD"
        ...
        nome_arquivo = f"{id_processo_novo} - Documento de Formalizacao de Demanda.{tipo}"
...
class GerarManifestoIRP(QDialog):
    def __init__(self, main_app, config_manager, df_registro, parent=None):
        super().__init__(parent)
        self.main_app = main_app
        self.config_manager = config_manager 
        ...
        self.setWindowTitle("Manifesto da Intenção de Registro de Preços (IRP)")
        self.setFixedSize(850, 410)
        self.pasta = ''
    ...
    def setupGrupoSIGDEM(self):
        ...
        layout.addWidget(labelAssunto)
        textEditAssunto = QTextEdit()
        textEditAssunto.setPlainText(f"{tipo_abreviado} {self.numero}/{self.ano} – Manifesto da Intenção de Registro de Preços (IRP)")
        textEditAssunto.setMaximumHeight(50)
        btnCopyAssunto = QPushButton("Copiar")
        btnCopyAssunto.clicked.connect(lambda: self.copyToClipboard(textEditAssunto.toPlainText()))
        layoutHAssunto = QHBoxLayout()
    ...
    def editarTemplate(self):
        template_path = TEMPLATE_PLANEJAMENTO_DIR / "template_manifesto_irp.docx"
        try:
            if sys.platform == "win32":
                subprocess.run(["start", "winword", str(template_path)], check=True, shell=True)
            elif sys.platform == "darwin":  # macOS
    ...
    def gerarDocumento(self, tipo="docx"):
        ...
        template_path = TEMPLATE_PLANEJAMENTO_DIR / f"template_manifesto_irp.{tipo}"
        objeto = remover_caracteres_especiais(self.df_registro['objeto'].iloc[0])
        id_processo_original = self.df_registro['id_processo'].iloc[0]
        id_processo_novo = id_processo_original.replace('/', '-')

        nome_pasta = f"{id_processo_novo} - {objeto}"
        subpasta_autorizacao = f"{id_processo_novo} - IRP"
        ...
        # Formatar o nome do arquivo
        nome_arquivo = f"{id_processo_novo} - Manifesto da Intenção de Registro de Preços.{tipo}"
        save_path = os.path.join(subpasta_destino, nome_arquivo)
