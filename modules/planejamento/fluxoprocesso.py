...
class FluxoProcessoDialog(QDialog):
    ...
    def generate_excel(self):
        # Atualizar as informações de self.df_processos
        df_processos = carregar_dados_processos(self.database_path)

        # Cria uma instância de ReportDialog com o DataFrame atualizado
        report_dialog = ReportDialog(df_processos, str(self.icons_dir), self)
...
class CustomListWidget(QListWidget):
    ...
    def clearClickEffect(self):
        ...
        if item:
            widget = self.itemWidget(item)
            if widget:
                widget.setStyleSheet("background-color: #91948A; color: black;")
    ...
    def applyClickEffect(self, item):
        ...
        if self.last_selected_item:
            ...
            if last_widget:
                last_widget.setStyleSheet("background-color: #91948A;")  # Estilo original
