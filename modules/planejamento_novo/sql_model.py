...
class SqlModel:
    ...
    def adjust_table_structure(self):
        ...
        if not query.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='controle_processos'"):
            print("Erro ao verificar existência da tabela:", query.lastError().text())
        if not query.next():
            print("Tabela 'controle_processos' não existe. Criando tabela...")
            self.create_table_if_not_exists()
        else:
            print("Tabela 'controle_processos' existe. Verificando estrutura da coluna...")
    ...
    def create_table_if_not_exists(self):
        ...
        if not query.exec("""
            CREATE TABLE IF NOT EXISTS controle_processos (
                ...
                data_limite_confirmacao_irp DATE,
                num_irp TEXT,
                om_participantes TEXT
            )
        """):
            print("Falha ao criar a tabela 'controle_processos':", query.lastError().text())
        else:
            print("Tabela 'controle_processos' criada com sucesso.")
