...
from modules.planejamento.utilidades_planejamento import DatabaseManager, carregar_dados_pregao, carregar_dados_licitacao
from modules.dispensa_eletronica.utilidades_dispensa_eletronica import ExportThread
from modules.planejamento_novo.sql_model import SqlModel, CustomTableView
from modules.planejamento_novo.menu.add_item import AddItemDialog
from modules.planejamento_novo.menu.salvar_tabela import SaveTableDialog
from modules.planejamento_novo.menu.graficos import GraficTableDialog
from modules.planejamento_novo.menu.gerar_tabela import TabelaResumidaManager
import pandas as pd
import os
import subprocess
import logging
...
class PlanejamentoNovoWidget(QMainWindow):
    ...
    def __init__(self, icons_dir, parent=None):
        ...
        self.output_path = os.path.join(os.getcwd(), "controle_licitacao.xlsx")
        self.dataUpdatedPlanejamento.connect(self.refresh_model)
    ...
    def init_model(self):
        ...
        return sql_model.setup_model("controle_processos", editable=True)
        ...
        if filepath:
            try:
                ...
                if 'status' in df.columns:
                    pass  # Mantém a coluna 'status' como está
                elif 'Status' in df.columns:
                    # Se a coluna 'Status' existir, renomeia para 'status'
                    df['status'] = df['Status']
                    df.drop(columns=['Status'], inplace=True)
                else:
                    # Se nenhuma das colunas existir, adiciona 'status' com valor padrão 'Planejamento'
                    df['status'] = 'Planejamento'

                # Lista de valores válidos para a coluna 'status'
                valid_situations = ["Planejamento", "Aprovado", "Sessão Pública", "Homologado", "Empenhado", "Concluído", "Arquivado"]

                # Verifica se os valores na coluna 'status' são válidos, senão, define como 'Planejamento'
                df['status'] = df['status'].apply(lambda x: x if x in valid_situations else 'Planejamento')
    ...
    def save_to_database(self, data, delete=False):
        with self.database_manager as conn:
            cursor = conn.cursor()
            if delete:
                cursor.execute("DELETE FROM controle_processos WHERE id_processo = ?", (data['id_processo'],))
            else:
                # Lista de valores válidos para a coluna 'status'

                valid_situations = ["Planejamento", "Consolidar Demandas", "Montagem do Processo", "Nota Técnica", "AGU", "Recomendações AGU", "Sessão Pública",  "Assinatura Contrato", "Concluído"]
                upsert_sql = '''
                INSERT INTO controle_processos (
                    id_processo, nup, objeto, uasg, tipo, numero, ano, sigla_om, setor_responsavel, 
                    material_servico, orgao_responsavel, status
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                if isinstance(data, pd.DataFrame):
                    # Atualiza 'status' para cada linha, se necessário
                    data['status'] = data['status'].apply(get_valid_situacao)
                    ...
                    data['status'] = get_valid_situacao(data.get('status', ''))
                    cursor.execute(upsert_sql, (
                        data['id_processo'], data['nup'], data['objeto'], data['uasg'],
                        data['tipo'], data['numero'], data['ano'],
                        data['sigla_om'], data.get('setor_responsavel', ''), 
                        data['material_servico'], data['orgao_responsavel'], data['status']
                    ))
            conn.commit()
        self.dataUpdatedPlanejamento.emit()
...
class CustomItemDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        if index.column() == 4:  # Verifica se é a coluna "Status"
            situacao = self.model.data(self.model.index(index.row(), self.model.fieldIndex('status')), Qt.ItemDataRole.DisplayRole)

            # Usando chaves que correspondem aos nomes sem extensão
            icon_key = {
                'Planejamento': 'business',
                'Consolidar Demandas': 'business',
                'Montagem do Processo': 'business',
                'Nota Técnica': 'business',
                'AGU': 'business',
                'Recomendações AGU': 'business',
                'Pré-Publicação': 'business',
                'Sessão Pública': 'session',
                'Assinatura Contrato': 'business',
                'Concluído': 'aproved'
            }.get(situacao)
