...
class TabelaDeRiscos:
    ...
    def configurar_paisagem(self):
        ...
        print("Seção em paisagem configurada com margens estreitas")

    def inserir_titulo(self, titulo):
        # Adiciona um título antes de inserir a tabela
    ...
    def inserir_tabela(self, df):
        # Salva o DataFrame em um arquivo Excel temporário
        ...
        # Insere a tabela do Excel no documento Word
        ...
        for i, row in enumerate(ws.iter_rows(values_only=True)):
            for j, value in enumerate(row):
                ...
                elif i % 2 == 1:  # Linhas ímpares
                    shading_elm = parse_xml(r'<w:shd {} w:fill="E7E6E6"/>'.format(nsdecls('w')))
                    cell._tc.get_or_add_tcPr().append(shading_elm)
                print(f"Celula [{i},{j}] ajustada com valor: {value}")
    ...
    def gerar_documento(self):
        self.criar_tabela_excel()  # Cria a tabela Excel
        self.carregar_template_word()
        self.configurar_paisagem()

        # Subtítulos e divisões baseadas na fase
        fases = {
        ...
        for titulo, dados_etapa in fases.items():
            if dados_etapa:  # Se houver dados para a fase
