...
from modules.controle_contratos.atualizar_dados_contratos import AtualizarDadosContratos
from modules.controle_contratos.utils_contratos import *
from modules.controle_contratos.gerar_tabela import *
...
class CustomTableModel(QStandardItemModel):
    def __init__(self, dados, colunas, icons_dir, parent=None):
        ...
        self.colunas = colunas
        self.icon_mapping = {
            'Status5': "icon_signature.png",
            'Status4': "icon_law_agu.png",
            'Status3': "icon_law.png",
            'Status2': "icon_tick.png",
            'Status1': "icon_send.png",
            'Status0': "icon_send.png",
    ...
    def setupIconItem(self, i, row):
        icon_path = self.determineIconPath(row)
        if icon_path:  # Verifica se um caminho foi retornado
    ...
    def determineIconPath(self, row):
        """Determina o caminho do ícone com base nas condições de 'row'."""
        # Utiliza o mapeamento existente para encontrar o nome do arquivo de ícone
        for status_key, icon_file in self.icon_mapping.items():
            if status_key in row and self.hasDateAtEnd(row[status_key]):
                return os.path.join(self.icons_dir, icon_file)

        # Determina o ícone baseado no valor de 'Dias'
        dias_value = int(row.get('Dias', 180))
        if dias_value < 60:
            return os.path.join(self.icons_dir, self.icon_mapping['Alert'])
        elif dias_value < 180:
            return os.path.join(self.icons_dir, self.icon_mapping['Warning'])
        else:
            return os.path.join(self.icons_dir, self.icon_mapping['Checked'])

    @staticmethod
    def hasDateAtEnd(text):
        """Verifica se o texto termina com uma data no formato dd/mm/aaaa."""
        return re.search(r"em \d{2}/\d{2}/\d{4}$", text) is not None
